...
1. 下載並解壓縮 `translate-web-by-browser-ai-v0.8.3.zip`。
...
Manifest V3 service worker
  ├─ 第一個快速批次最多 3 段、約 650 字元，優先處理當下可視區
  ├─ 其餘可視區段落仍排在畫面外內容之前
...
build 會更新可直接載入且納入版本控制的 `extension/`，並複製發布內容至 `dist/extension/`。發布 ZIP 位於 `dist/release/translate-web-by-browser-ai-v0.8.3.zip`。
...
- 第一個快速批次最多 3 段、約 650 個來源字元，從目前視窗最接近上方的內容開始；其餘可見段落仍優先於畫面外內容。這可降低 M365 第一段中文出現前的等待時間，而不犧牲後續整頁翻譯。
